...
async def create_quest(quest_data: QuestCreateSchema):
    logger.debug(f"Attempting to create quest with id: {quest_data.id}")
    try:
        new_quest = Quest(
            id=quest_data.id,
        ...
        logger.info(f"Quest '{new_quest.id}' titled '{new_quest.title}' created successfully.")
        return QuestResponseSchema.model_validate(new_quest)
    except ValueError as e:
        logger.warning(f"Failed to create quest (ID: {quest_data.id}): {e}") 
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(f"Unexpected error creating quest (ID: {quest_data.id}): {e}", exc_info=True)
...
async def get_available_quests():
    logger.debug("Fetching available quests.")
...
async def complete_quest_endpoint(quest_id: str):
    logger.debug(f"Attempting to complete quest: {quest_id}")
    try:
        ...
        logger.info(f"Quest '{quest_id}' marked as completed successfully via API.")
        return QuestResponseSchema.model_validate(completed_quest)
    except ValueError as e: 
        logger.warning(f"Attempt to complete quest failed. Quest ID '{quest_id}' not found: {e}")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except PermissionError as e: 
        logger.warning(f"Attempt to complete quest '{quest_id}' failed due to unmet dependencies: {e}")
...
async def get_quest_by_id(quest_id: str):
    logger.debug(f"Fetching quest by ID: {quest_id}")
    quest = quest_manager.get_quest(quest_id)
    if not quest:
        logger.warning(f"Quest with ID '{quest_id}' not found during API call.")
...
async def check_for_cycles():
    ...
    logger.info(f"Cycle check result: {message}")
...
async def get_completion_order_api():
    logger.debug("Attempting to get completion order.")
    try:
        order = quest_manager.get_completion_order()
        logger.info("Successfully retrieved completion order.")
        return CompletionOrderResponseSchema(order=order, message="Successfully retrieved completion order.")
    except ValueError as e: 
        logger.warning(f"Cannot get completion order, graph contains cycles: {e}")
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(e))
    except RuntimeError as e: 
        logger.error(f"Runtime error getting completion order: {e}", exc_info=True)
...
async def save_quests_to_file(payload: FilePathSchema):
    logger.info(f"Attempting to save quests to file: {payload.filepath}")
    try:
        quest_manager.save_quests(payload.filepath) 
        logger.info(f"Quests successfully saved to {payload.filepath} via API call.")
        return {"message": f"Quests successfully saved to {payload.filepath}"}
    except (IOError, TypeError) as e:
        logger.error(f"Error saving quests to {payload.filepath}: {e}", exc_info=True)
...
async def load_quests_from_file(payload: FilePathSchema):
    logger.info(f"Attempting to load quests from file: {payload.filepath}")
    try:
        ...
        logger.warning(f"File not found for loading quests: {payload.filepath}. Error: {e}")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Error loading quests: {str(e)}")
    except (IOError, ValueError) as e: 
        logger.error(f"Error loading quests from {payload.filepath}: {e}", exc_info=True)
        ...
        logger.error(f"Unexpected error loading quests from {payload.filepath}: {e}", exc_info=True)
...
async def reset_state_testing():
    ...
    quest_manager = QuestManager()
    logger.info(f"QuestManager state has been reset for testing via API.")
    try:
        if os.path.exists(DEFAULT_QUEST_FILE):
            logger.info(f"Attempting to load initial quests from {DEFAULT_QUEST_FILE} after reset...")
            quest_manager.load_quests(DEFAULT_QUEST_FILE)
        else:
            logger.info(f"Default quest file {DEFAULT_QUEST_FILE} not found after reset. Manager is empty.")
    except Exception as e:
        logger.warning(f"Could not load initial quests from {DEFAULT_QUEST_FILE} after reset. Error: {e}", exc_info=True)
